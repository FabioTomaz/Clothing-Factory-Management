...
AS
RETURN(
	SELECT top 1 REFERENCIA, ID, SUM(QUANTIDADE) AS QTDPRODUTO FROM ENCOMENDA
	JOIN CONTEUDO_ENCOMENDA ON ENCOMENDA.N_ENCOMENDA=ENCOMENDA.N_ENCOMENDA 
	JOIN [PRODUTO-PERSONALIZADO] ON [PRODUTO-PERSONALIZADO].REFERENCIA=CONTEUDO_ENCOMENDA.REFERENCIA_PRODUTO 
	AND [PRODUTO-PERSONALIZADO].ID=CONTEUDO_ENCOMENDA.ID_PRODUTO
	WHERE Year(DATA_ENTREGA) = Year(CURRENT_TIMESTAMP) AND Month(DATA_ENTREGA) = Month(CURRENT_TIMESTAMP)
	GROUP BY REFERENCIA, ID
	ORDER BY  SUM(QUANTIDADE) DESC
)
GO

--DROP FUNCTION DBO.getProdutoMaisVendidoMes
...
--DROP FUNCTION DBO.getTotalProdutosVendidosMes

CREATE FUNCTION dbo.getTotalProdutosVendidos() RETURNS INT
AS
BEGIN
	DECLARE @TOTAL INT;
	SELECT @TOTAL=SUM(QUANTIDADE) FROM CONTEUDO_ENCOMENDA; 
	RETURN @TOTAL;
END
GO
